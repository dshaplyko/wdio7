...
export class Waiter {
  ...
  public static async waitForDisplayed(el: WebdriverIO.Element, reverse?: boolean, maxWaitTime: number = Timeouts.max) {
    try {
      await (await el).waitForDisplayed({ timeout: maxWaitTime, reverse: reverse });
    } catch (e) {
      if (e.name === "TimeoutError") {
